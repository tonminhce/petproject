 ...
 * D4). Deliberately does NOT extend {@code BaseKafkaListenerConfig} — that
 * base hard-wires a {@code JsonDeserializer} for the value, which would bind
 * the wire payload to a DTO. T4 GATE: the fleet producer serializes the
 * outbox payload STRING via {@code JsonKafkaSerializer}, so records arrive
 * DOUBLE-ENCODED (a JSON string token wrapping the event JSON) — binding that
 * directly to a DTO throws on the token. The value therefore deserializes as
 * RAW STRING ({@link StringDeserializer}, never failing on poison bytes) and
 * {@link MediaDeletedConsumer} performs the unwrap + typed bind itself, which
 * also stays tolerant of a future single-encoded relay.
 *
