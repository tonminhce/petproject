...
/**
 * D4 relay: polls PENDING outbox rows in insertion order and publishes them
 ...
 * FAILED) and is retried next cycle — containment fleet posture.
 *
...
public class MediaOutboxRelay {
    ...
    public void relay() {
        ...
        log.info("Relaying {} media outbox event(s)", pending.size());
        for (OutboxEvent event : pending) {
            try {
                ...
                if (event.getRetryCount() >= maxRetries) {
                    ...
                    log.error("Media outbox event {} permanently failed", event.getEventId(), ex);
                } else {
