...
import com.shop.notificationservice.config.TestLiquibaseConfig;
...
/**
 * Base class for notification-service full-context integration tests — the
 * property set mirrors promotion-service's {@code AbstractIntegrationTest}
 * (same singleton container lifecycle) but keeps BOTH containers: this
 ...
 *       chain entirely.</li>
 *   <li>SMTP stays disabled ({@code shop.notification.smtp.enabled} defaults
 *       to {@code false}), so the {@code LoggingNotificationSender} primary is
 *       active and no mail host is needed.</li>
 * </ul></p>
 *
